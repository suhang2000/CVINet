# -*- coding: utf-8 -*-
# Vendored from PySODMetrics: https://github.com/lartpang/PySODMetrics
# Copyright (c) 2020 lartpang. Released under the MIT License; see LICENSE.
import warnings
...
class Fmeasure(object):
    ...
    def cal_pr(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        ...
        """
        # 1. Histograms of the prediction over GT foreground/background regions
        pred = (pred * 255).astype(np.uint8)
        bins = np.linspace(0, 256, 257)
        fg_hist, _ = np.histogram(pred[gt], bins=bins)  # the last bin is [255, 256]
        bg_hist, _ = np.histogram(pred[~gt], bins=bins)
        # 2. Cumulative histograms (flip + cumsum) give, for every threshold at once,
        # the number of pixels predicted >= that threshold in each region
        fg_w_thrs = np.cumsum(np.flip(fg_hist), axis=0)
        bg_w_thrs = np.cumsum(np.flip(bg_hist), axis=0)
        # 3. Precision and recall share the numerator (pred==1 & gt==1) and differ
        # only in the denominator: pred==1 for precision, gt==1 for recall
        TPs = fg_w_thrs
        Ps = fg_w_thrs + bg_w_thrs
        # Guard against division by zero: a zero denominator implies a zero
        # numerator here, so setting it to 1 keeps the result exact
        Ps[Ps == 0] = 1
...
class WeightedFmeasure(object):
    ...
    def cal_wfm(self, pred: np.ndarray, gt: np.ndarray) -> float:
        ...
        # Note: index with gt == 1 rather than Ew[gt]; the latter would only index dim 0
        R = 1 - np.mean(Ew[gt == 1])
        P = TPw / (TPw + FPw + EPS)
